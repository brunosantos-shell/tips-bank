...
log_format = '{"ts":"%(asctime)s","level":"%(levelname)s","service":"api-transacoes","msg":"%(message)s"}'

logging.basicConfig(
    level=os.getenv("LOG_LEVEL", "INFO"),
    format=log_format,
    handlers=[
        logging.StreamHandler(),        # stdout (kubectl logs)
        logging.FileHandler(LOG_FILE)   # arquivo para sidecar
    ],
)
